...
DistanceFacadeCompartments = DMN('C:/Users/camil/OneDrive - UGent/Ugent/AJ/23-24/Masterthesis/MODELS/DMN/3.Voorschriften voor sommige bouwelementen/final/3-5-1-2_DistanceBetweenWallsOfDifferentCompartments_v2.dmn')
...
def calculate_expression(alpha):
    if 0 <= alpha <= 90:
        CosineCalcValue = 1 + 7 * np.cos(np.radians(alpha))
    elif 90 < alpha <= 180:
        CosineCalcValue = 1
    else:
        CosineCalcValue = None
    return CosineCalcValue

#set value for enclosed corner
EnclosedCornerBetweenTheFacades = 0.1

#Value for 'EnclosedCornerBetweenTheFacades' also required here
CosineCalcValue = round(calculate_expression(EnclosedCornerBetweenTheFacades),2)
...
DistanceFacadeCompartments.set_value('CosineCalc', CosineCalcValue)
